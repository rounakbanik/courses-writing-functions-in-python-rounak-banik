...
`@part1`
- Arguments are mapped to the function's parameters based on their position. {{1}}
...
`@script`
Positional arguments are those that are mapped to the function's parameters based on their position. Therefore, these arguments are order dependent.
...
- Makes function calling easier and enhances readability of code.{{3}}
...
Therefore, it makes function calling easier and enhances code readability as parameter names are visible in the function call and the arguments are order independent.
...
`@script`
The arguments that we've seen so far were required arguments. In other words, the function would throw an error if we didn't pass them.
...
`@script`
That's enough theory for now. Let us now check if we have understood the concepts explored in this chapter through a few practice exercises.
